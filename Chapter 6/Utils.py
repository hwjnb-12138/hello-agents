...
def get_chinese_name(character: str) -> str:
    if character in CHINESE_NAMES:
        return character
    return random.choice(CHINESE_NAMES)

def format_player_list(players: List[AgentBase], show_role: bool = False):
    if not players:
        return "暂无玩家"
    
    if show_role:
        return "、".join([f"{player.name}({getattr(player, 'role', '未知角色')})" for player in players])
    else:
